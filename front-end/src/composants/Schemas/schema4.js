...
function Schema1({ trous, ranges }) {
  ...
  function generateSchema(rows, columns) {
    ...
    for (let i = 1; i < columns; i++) {
      table[0][i] = table[0][i - 1] + increment3;
    }

    for (let row = 1; row < rows; row++) {
      for (let col = 0; col < columns; col++) {
        if (row % 4 === 0) {
          table[row][col] = table[row - 1][col] + increment2;
        } else {
            ...
            {matrix.map((row, rowIndex) => (
              <tr key={rowIndex}>
                ...
                {row.map((cell, cellIndex) => {
                  let arrowSymbol = "";
                  if (rowIndex === 0) {
                    arrowSymbol = <span className="black">→</span>;
                  } else if (rowIndex % 4 === 0) {
                    arrowSymbol = <span className="green-down-left">↓</span>; // Utilise arrowSymbol avec increment2
                  } else {
                    arrowSymbol = <span className="yellow-down-left">↓</span>;
                  }
